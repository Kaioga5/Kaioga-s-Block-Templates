...
import { BlockComponentRandomTickEvent, Vector3, system } from "@minecraft/server";
...
const WALL_SHIFT: Record<string, [number, number]> = {
  north: [0, 1],
  south: [0, -1],
  east: [-1, 0],
  west: [1, 0],
...
function flamePosition(x: number, y: number, z: number, face: string): Vector3 {
  ...
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:torch_particles",
    {
      ...
      onRandomTick(event: BlockComponentRandomTickEvent): void {
        ...
        const { block } = event;
        const face = String(
          block.permutation.getAllStates()["minecraft:block_face"],
        );
