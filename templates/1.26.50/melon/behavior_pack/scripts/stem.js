...
import { ATTACHED_STATE, FARMLAND_ID, GROWTH_STATE, MAX_GROWTH, MELON_ID, MIN_LIGHT, MOISTURE_STATE, POINTS_DRY, POINTS_DRY_NEIGHBOUR, POINTS_WET, POINTS_WET_NEIGHBOUR, STEM_ID, fruitSoilIds, } from "./config.js";
...
export function attachedOf(block) {
    const value = block.permutation.getAllStates()[ATTACHED_STATE];
    return typeof value === "string" ? value : "none";
}
// Write both states back at once
function setStem(block, growth, attached) {
    const states = block.permutation.getAllStates();
    block.setPermutation(BlockPermutation.resolve(STEM_ID, {
        ...states,
        [GROWTH_STATE]: growth,
...
export function advance(block, stages) {
...
function farmlandPoints(block, wet, dry) {
...
function growthScore(block) {
    ...
    for (let dx = -1; dx <= 1; dx++) {
        for (let dz = -1; dz <= 1; dz++) {
            ...
            score += farmlandPoints(soil?.offset({ x: dx, y: 0, z: dz }), POINTS_WET_NEIGHBOUR, POINTS_DRY_NEIGHBOUR);
        }
    }
    // Crowding: the same crop on a diagonal, or on both axes at once, halves
    // the score
    const diagonal = sameCrop(block.offset({ x: 1, y: 0, z: 1 })) ||
...
export function straighten(block) {
