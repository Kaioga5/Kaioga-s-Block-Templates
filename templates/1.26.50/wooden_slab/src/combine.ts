...
function makeDouble(block: Block, player: Player | undefined): void {
  const states = block.permutation.getAllStates();
  block.setPermutation(
    BlockPermutation.resolve(block.typeId, { ...states, [DOUBLE_STATE]: true }),
  );
...
function onPlayerBreak(event: BlockComponentPlayerBreakEvent): void {
  ...
  event.dimension.spawnItem(
    new ItemStack(event.brokenBlockPermutation.type.id),
...
function onUseOn(event: ItemComponentUseOnEvent): void {
  ...
  if (isHalfSlabOf(target, slabId)) {
    ...
    if (
      (half === "bottom" && blockFace === Direction.Up) ||
...
system.beforeEvents.startup.subscribe((init) => {
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:wooden_slab_combine",
    {
      beforeOnPlayerPlace,
      onPlayerBreak,
    },
