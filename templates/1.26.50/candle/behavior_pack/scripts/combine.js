// Adds a candle to a cluster that is already standing.
//
// The same component is registered twice, the way this module's slabs merge:
...
// to the block placer and places a new candle.
import { Direction, EquipmentSlot, GameMode, Player, system, } from "@minecraft/server";
import { CANDLE_ID, MAX_CANDLES, candleCount, isLit, setCandle, } from "./candle.js";
// Vanilla plays this when a candle joins a cluster
const ADD_SOUND = "use.candle";
// Cells where a candle was placed normally in the last few ticks. The item
...
function clickedThrough(block, face) {
    switch (face) {
        case Direction.Up:
            return block.below();
        case Direction.Down:
...
// True if the block is a cluster of this candle with room for one more
function hasRoom(block) {
    return (block !== undefined &&
        block.typeId === CANDLE_ID &&
        candleCount(block) < MAX_CANDLES);
}
// Take one candle out of the player's hand, except in creative
...
// Put one more candle on the cluster, keep it lit or unlit as it was, and pay
function addCandle(block, player) {
    setCandle(block, candleCount(block) + 1, isLit(block));
...
// and the engine wants to put a new candle in the cell on the far side.
...
// because the cell on the far side of the clicked face is occupied, or because
// the placement filter refused it. The clicked block itself is the candidate
function onUseOn(event) {
    ...
    system.runTimeout(() => {
        ...
        addCandle(block, source instanceof Player ? source : undefined);
    }, 1);
}
// Register the same component name in both registries before world load
system.beforeEvents.startup.subscribe((init) => {
    init.blockComponentRegistry.registerCustomComponent("kai_templates:candle_combine", {
        beforeOnPlayerPlace,
    });
    init.itemComponentRegistry.registerCustomComponent("kai_templates:candle_combine", {
        onUseOn,
    });
});
