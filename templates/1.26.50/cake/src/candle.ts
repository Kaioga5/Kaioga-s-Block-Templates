...
export function isCandle(itemId: string | undefined): boolean {
...
function dropCandle(block: Block): void {
  ...
  if (cakeBlock !== undefined && block.typeId === CAKE_ID && biteCount(block) === 0) {
    ...
    system.run(() => {
      ...
      block.setPermutation(
        BlockPermutation.resolve(cakeBlock, { [LIT_STATE]: false }),
      );
...
system.beforeEvents.startup.subscribe((init) => {
  ...
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:candle_cake_particles",
    {
  ...
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:candle_cake_interact",
    {
      onPlayerInteract(event: BlockComponentPlayerInteractEvent): void {
        ...
        block.setPermutation(
          BlockPermutation.resolve(CAKE_ID, { [BITE_STATE]: 1 }),
        );
      },
    },
  );
