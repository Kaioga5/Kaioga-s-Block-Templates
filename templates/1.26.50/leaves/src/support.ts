...
import { Block, BlockPermutation, LocationOutOfWorldBoundariesError, Vector3 } from "@minecraft/server";
import { DISTANCE_STATE, LEAVES_ID, MAX_DISTANCE, UPDATE_STATE, supportIds, supportTags } from "./config.js";

// The states of one leaf, as getAllStates hands them back
export type States = Record<string, boolean | number | string>;

// The six directions a leaf can be attached through
export const NEIGHBOURS: Vector3[] = [
    { x: 1, y: 0, z: 0 },
    { x: -1, y: 0, z: 0 },
    { x: 0, y: 1, z: 0 },
    { x: 0, y: -1, z: 0 },
...
export function isSupport(block: Block): boolean {
...
export function wasSupport(permutation: BlockPermutation): boolean {
...
export function wasLeaf(permutation: BlockPermutation): boolean {
    return permutation.matches(LEAVES_ID);
}

// The distance a leaf currently holds
export function readDistance(states: States): number {
...
export function isVerified(states: States): boolean {
    return states[UPDATE_STATE] === false;
}

// What one look at the six neighbours found
export interface Survey {
    // The distance this leaf should hold now
    distance: number;
    // Where the leaf neighbours are, so a change here can be passed on to them
    leafNeighbours: Vector3[];
    // True when one of those neighbours has never been measured, so this
    // answer may still improve once it has been
    unsettled: boolean;
}
...
export function survey(block: Block): Survey | undefined {
...
export function writeDistance(block: Block, states: States, distance: number): void {
    block.setPermutation(
        BlockPermutation.resolve(LEAVES_ID, {
            ...states,
