...
// blocks do not get at 1.26.50, and a custom block state holds at most sixteen
...
const BLOCK_BY_ITEM = new Map(
  POTTED_PLANTS.map((plant) => [plant.item, plant.block]),
);

// Potted block to the item it hands back
const ITEM_BY_BLOCK = new Map(
  POTTED_PLANTS.map((plant) => [plant.block, plant.item]),
);
...
system.beforeEvents.startup.subscribe((init) => {
  ...
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:flower_pot_plant",
    {
