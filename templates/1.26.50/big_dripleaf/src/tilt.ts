...
import { Block, BlockComponentRedstoneUpdateEvent, BlockComponentStepOnEvent, BlockPermutation, system } from "@minecraft/server";
import { DRIPLEAF_ID, HEAD_STATE, TILT_STATE, tiltSequence } from "./config.js";

function readTilt(block: Block): string {
    const value = block.permutation.getAllStates()[TILT_STATE];
    return typeof value === "string" ? value : "none";
}

function isHead(block: Block): boolean {
    return block.permutation.getAllStates()[HEAD_STATE] === true;
}

function setTilt(block: Block, tilt: string): void {
...
const TILT_SOUNDS = new Map<string, string>([
...
function step(block: Block, expected: string): void {
