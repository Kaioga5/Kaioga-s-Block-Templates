...
system.beforeEvents.startup.subscribe((init) => {
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:shelf_mushroom_bounce",
    {
      onEntityFallOn(event: BlockComponentEntityFallOnEvent): void {
        ...
        const strength = Math.min(
          BOUNCINESS * IMPACT_PER_SQRT_BLOCK * Math.sqrt(event.fallDistance),
          MAX_LAUNCH,
        );
...
world.beforeEvents.entityHurt.subscribe((event) => {
  ...
  event.damage = Math.max(
    0,
    Math.ceil(distance * FALL_DISTANCE_KEPT - SAFE_FALL_BLOCKS),
  );
});
