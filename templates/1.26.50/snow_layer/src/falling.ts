...
// where it cannot land. Custom blocks have no native gravity at 1.26.50, so
...
import { Block, BlockPermutation, Entity, ItemStack, system, world } from "@minecraft/server";
import {
    DROP_PER_LAYER,
    FALL_AGE_PROPERTY,
    FALL_HEIGHT_PROPERTY,
    FALLING_ID,
    HEIGHT_STATE,
    LAND_SOUND,
    MAX_HEIGHT,
...
function isSnow(block: Block | undefined): block is Block {
...
function carriedBy(faller: Entity): number {
    const height = faller.getProperty(FALL_HEIGHT_PROPERTY);
    return typeof height === "number" ? height : 0;
}

function drift(height: number): BlockPermutation {
...
function isSupported(block: Block): boolean {
...
function drop(block: Block): void {
...
export function dropIfUnsupported(block: Block): boolean {
...
function cellOf(faller: Entity): Block | undefined {
...
function land(faller: Entity, cell: Block): void {
...
function landed(faller: Entity): void {
...
function sweepFaller(faller: Entity): void {
    const age = ((faller.getDynamicProperty(FALL_AGE_PROPERTY) as number) ?? 0) + SWEEP_INTERVAL;
...
// it
function floorWent(changed: Block): void {
    system.run(() => {
        try {
