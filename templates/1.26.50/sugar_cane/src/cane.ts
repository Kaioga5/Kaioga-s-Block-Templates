...
export function isCane(block: Block | undefined): block is Block {
    return block !== undefined && block.typeId === CANE_ID;
}

export function ageOf(block: Block): number {
    const value = block.permutation.getAllStates()[AGE_STATE];
    return typeof value === "number" ? value : 0;
}

export function setAge(block: Block, age: number): void {
...
// stays alive
export function hasWaterBeside(block: Block | undefined): boolean {
    if (block === undefined) {
        return false;
    }
    ...
    return sides.some(
        (side) =>
            side !== undefined && (waterIds.has(side.typeId) || side.isWaterlogged),
    );
}

// Is this block the bottom of a stand?
export function isRoot(block: Block): boolean {
...
export function rootOf(block: Block): Block {
    let current: Block = block;
    for (let step = 0; step < MAX_HEIGHT; step++) {
        const below: Block | undefined = current.below();
        if (!isCane(below)) {
            return current;
        }
        current = below;
...
export function heightOf(root: Block): number {
    let count = 1;
...
export function isTop(block: Block): boolean {
...
export function uproot(block: Block): void {
