// Handles copper maintenance on the bars: honeycomb waxes them while
// sneaking, an axe strips wax or scrapes one oxidation stage back. This
// listens to the interact-with-block before-event, the same mechanism the
// copper door, slab and trapdoor use, so all the copper templates share
// one precise item-use path instead of a generic interact handler.
import {
  Block,
...
import { STAGE_BY_ID, STAGE_BY_WAXED_ID } from "./family.js";
...
function spawnWaxMotes(block: Block, red: number, green: number, blue: number): void {
...
// Wax the bars: swap to the waxed twin, play the vanilla wax effects, and
// use up one honeycomb. setType is enough here, the engine recomputes the
// bar connections for the new block on its own
function applyWax(block: Block, waxedId: string, player: Player): void {
  block.setType(waxedId);
...
world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
  ...
  // Honeycomb waxes unwaxed bars, but only while sneaking, the
  // deliberate crouch keeps waxing from firing during normal clicking
  // around the bars
  if (held.typeId === "minecraft:honeycomb" && unwaxedStage !== undefined) {
    if (!player.isSneaking) {
      return;
    }
    // Cancel the interaction and do the world write after the
    // read-only before-event window
    event.cancel = true;
    system.run(() => {
