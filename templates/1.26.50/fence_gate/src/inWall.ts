...
function sideNeighbors(block: Block, facing: string): Array<Block | undefined> {
...
function isWall(block: Block | undefined): boolean {
  if (block === undefined) {
    return false;
  }
...
function updateInWall(block: Block): void {
...
function updateAdjacentGates(changed: Block): void {
  const neighbors = [
    changed.north(),
    changed.south(),
    changed.east(),
...
system.beforeEvents.startup.subscribe((init) => {
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:fence_gate_in_wall",
    {
