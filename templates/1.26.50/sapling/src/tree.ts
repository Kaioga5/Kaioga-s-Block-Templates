...
import { Block, BlockPermutation, Dimension, Vector3, world } from "@minecraft/server";
...
function isClear(block: Block | undefined): boolean {
...
function hasRoom(origin: Block, trunk: number): boolean {
...
function placeSoftly(dimension: Dimension, location: Vector3, permutation: BlockPermutation): void {
    try {
...
function buildShape(origin: Block): void {
...
function buildStructure(origin: Block, name: string): void {
    const { dimension, location } = origin;
    try {
...
export function growTree(origin: Block): void {
