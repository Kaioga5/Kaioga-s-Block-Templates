...
function settle(faller: Entity, blockId: string, asItem: boolean): void {
...
function onReport(faller: Entity, message: string): void {
  // Read back what this entity should become
  const blockId =
    (faller.getDynamicProperty(PROP_BLOCK) as string) ?? DEFAULT_POWDER;
  const curedId =
    (faller.getDynamicProperty(PROP_CURED) as string) ??
...
function sweepFaller(faller: Entity): void {
  ...
  const age =
    ((faller.getDynamicProperty(PROP_AGE) as number) ?? 0) + SWEEP_INTERVAL;
