...
export function dropColumn(dimension: Dimension, tip: Vector3, thicknesses: string[]): void {
...
function land(faller: Entity): void {
...
function sweepFaller(faller: Entity): void {
    const age = ((faller.getDynamicProperty(AGE_PROPERTY) as number) ?? 0) + SWEEP_INTERVAL;
