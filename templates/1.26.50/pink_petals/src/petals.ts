...
export function petalCount(block: Block): number {
...
export function setPetalCount(block: Block, count: number): void {
    const states = block.permutation.getAllStates();
    block.setPermutation(
        BlockPermutation.resolve(PETALS_ID, { ...states, [PETALS_STATE]: count }),
    );
}

// Room for at least one more flower
export function hasRoom(block: Block | undefined): block is Block {
    return (
        block !== undefined &&
