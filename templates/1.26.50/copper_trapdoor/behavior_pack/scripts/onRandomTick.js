// Registers the component that weathers the copper trapdoor over time.
...
import { STAGE_BY_ID, swapTrapdoorType } from "./family.js";
...
system.beforeEvents.startup.subscribe((init) => {
    init.blockComponentRegistry.registerCustomComponent("kai_templates:copper_trapdoor_oxidize", {
        // Run this code on each random tick
        onRandomTick(event) {
            // Get the block from the event
            const { block } = event;
            // Find this block's stage and the stage after it. The final
            // stage has no next entry, so fully oxidized trapdoors stop here
            ...
            // Swap to the next stage's block, keeping facing, open and
            // top/bottom placement
            swapTrapdoorType(block, stage.next);
        },
    });
});
