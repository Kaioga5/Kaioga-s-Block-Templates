...
// support disappears. Custom blocks have no native gravity at 1.26.40, the
...
import { Direction, LiquidType, MolangVariableMap, system, world, } from "@minecraft/server";
import { concreteConversions, DEFAULT_DUST_COLOR, DUST_MAX, DUST_MIN, DUST_PARTICLE, dustColors, ENTITY_ID, PROP_AGE, PROP_BLOCK, PROP_CURED, } from "./config.js";
...
function shedDust(block) {
    ...
    for (let mote = 0; mote < count; mote++) {
        block.dimension.spawnParticle(DUST_PARTICLE, {
...
function touchesWater(block) {
    ...
    return neighbors.some((n) => n !== undefined && n.isLiquid && n.typeId.includes("water"));
}
...
system.beforeEvents.startup.subscribe((init) => {
    init.blockComponentRegistry.registerCustomComponent("kai_templates:concrete_powder_physics", {
