# Block Templates - 1.26.40

This module contains 71 block templates for Minecraft Bedrock Edition 1.26.40,
covering building blocks, plants, redstone, lighting and interactive blocks. All
of them run on the current stable platform, and none require an experimental
toggle.
...
- **Stable APIs only.** Traits (`placement_direction`, `placement_position`,
  ...
  `@minecraft/server` 2.9.0, nothing behind a toggle.
...
| [Jukebox](jukebox/) | none | Yes | A jukebox that plays and ejects music discs, drops the disc when broken, shows notes and the track title, and reports the disc to a comparator. |
...
| [Note Block](note_block/) | none | Yes | A note block that steps its pitch on click, plays on redstone, picks its instrument from the block below, and goes silent under a block. |
...
| [Pointed Dripstone](pointed_dripstone/) | placement_position | Yes | Pointed dripstone that hangs or stands in five thicknesses, merges columns, drips water or lava into cauldrons, grows over time, and hurts whatever falls onto it. |
...
for scripted weathering. **Skulls, Flower Pot, Ladder, Grass Block and Concrete
Powder** each solve one hard problem and document the workaround.

## Requirements

- Minecraft Bedrock Edition 1.26.40.
- No experimental toggles for any template in this module.
- Scripted templates use `@minecraft/server` 2.9.0 (stable).
