// Handles copper maintenance on the trapdoor: honeycomb waxes it while
...
// own interact component, because waxing must NOT also swing the trapdoor,
...
import { STAGE_BY_ID, STAGE_BY_WAXED_ID, swapTrapdoorType } from "./family.js";
...
function applyScrape(
  ...
  swapTrapdoorType(block, targetId);
  const center = block.center();
...
world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
  ...
  // Honeycomb waxes an unwaxed trapdoor, but only while sneaking, the
  // deliberate crouch keeps waxing from firing during normal trapdoor use
  if (held.typeId === "minecraft:honeycomb" && unwaxedStage !== undefined) {
    if (!player.isSneaking) {
      return;
    }
    // Cancel so the trapdoor does not also toggle; the world write has
    // to wait until after the read-only before-event window
    event.cancel = true;
    system.run(() => {
  ...
  // packs work too. Axe use never toggles a copper trapdoor in vanilla
