// Stacking twisting vines by hand.
...
// turned into another segment on top of the stalk.
import { system, world } from "@minecraft/server";
import { VINE_ID } from "./config.js";
import { extend, findTip } from "./growth.js";
import { consumeOne } from "./hand.js";
// The template's blocks.json puts twisting vines on the weeping vines sound set
...
world.beforeEvents.playerInteractWithBlock.subscribe((event) => {
    ...
    system.run(() => {
        ...
        // Clicking anywhere on the stalk works on its growing end, so a player
        // does not have to line the cursor up with the tip
        const tip = findTip(block);
        // A hand-placed segment is not held back by the age cap the way random
        // growth is, so the stalk keeps going as long as there is room
        if (tip === undefined || !extend(tip, false)) {
            return;
        }
        block.dimension.playSound(PLACE_SOUND, (tip.above() ?? tip).center());
        consumeOne(player);
    });
});
