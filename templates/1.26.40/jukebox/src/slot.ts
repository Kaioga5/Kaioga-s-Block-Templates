...
import { Disc, discForItem, nowPlaying, signalFor } from "./discs.js";
...
function noteChannel(pitch: number, phase: number): number {
...
function noteColour(): MolangVariableMap {
...
const playing = new Map<
  string,
  { sound: SoundInstance; notes: number; ends: number }
>();
...
function discIn(block: Block): Disc | undefined {
...
function stopMusic(block: Block): void {
...
// nothing at all
function startMusic(block: Block, disc: Disc): void {
  stopMusic(block);
  const key = keyOf(block);
  ...
  const ends = system.runTimeout(
    () => stopMusic(block),
    Math.ceil(disc.seconds * 20),
  );
...
// two places at once
function ejectDisc(block: Block, disc: Disc): void {
...
function consumeOne(player: Player, held: ItemStack): void {
...
system.beforeEvents.startup.subscribe((init) => {
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:jukebox_slot",
    {
      onBreak(event: BlockComponentBlockBreakEvent): void {
