...
function smothers(above: Block | undefined): boolean {
  if (above === undefined) {
    return false;
  }
  if (
...
function randomNearbyOffset(): [number, number, number] {
    ...
    "kai_templates:grass_spread",
    {
      ...
      onRandomTick(event: BlockComponentRandomTickEvent): void {
        ...
        for (let attempt = 0; attempt < 4; attempt++) {
          ...
          const grownId =
            target === undefined ? undefined : spreadTargets.get(target.typeId);
