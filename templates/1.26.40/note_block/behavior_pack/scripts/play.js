...
import { BlockPermutation, MolangVariableMap, system, } from "@minecraft/server";
import { COLOUR_RANGE, NOTE_COUNT, NOTE_PARTICLE, PARTICLE_HEIGHT, instrumentFor, } from "./instruments.js";
...
function noteOf(block) {
    ...
    const value = (typeof octave === "number" ? octave : 0) * STEPS_PER_OCTAVE +
...
function setNote(block, note) {
    const states = block.permutation.getAllStates();
    block.setPermutation(BlockPermutation.resolve(NOTE_BLOCK_ID, {
        ...states,
...
function colourFor(note) {
    const position = note / COLOUR_RANGE;
    const wave = (offset) => Math.max(0, Math.sin((position + offset) * Math.PI * 2) * 0.65 + 0.35);
...
function spawnNoteParticle(block, note) {
...
function playNote(block) {
    if (block.above()?.isAir !== true) {
        return;
    }
    const note = noteOf(block);
    const sound = instrumentFor(block.below()?.typeId);
...
system.beforeEvents.startup.subscribe((init) => {
    init.blockComponentRegistry.registerCustomComponent("kai_templates:note_block_play", {
