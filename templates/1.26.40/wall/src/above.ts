...
const CONNECTION: Record<Side, string> = {
...
const VANILLA_CONNECTION: Record<Side, string> = {
...
const POST_OVERRIDE_IDS = new Set<string>([
  "minecraft:torch",
...
// What the block above rests on. The centre flag drives the post, the side
// flags raise one connected side each
interface Cover {
  centre: boolean;
  sides: Record<Side, boolean>;
  // Set only when the block above is a wall that is showing its post. Vanilla
  // short-circuits on that and always draws the post underneath
  forcesPost: boolean;
}
...
function connected(states: States, side: Side): boolean {
...
function structuralPost(states: States): boolean {
...
function coverFromArms(post: boolean, sides: Record<Side, boolean>): Cover {
  return {
...
function coverFromWall(above: Block): Cover {
...
function coverFromVanillaWall(states: States): Cover {
...
function isPostOverride(block: Block): boolean {
...
function coverAbove(wall: Block): Cover {
...
// out over it
function tallSides(states: States, cover: Cover): Record<Side, boolean> {
  return {
...
function postFor(cover: Cover, tall: Record<Side, boolean>): boolean {
...
function shapeFor(wall: Block): { post: boolean; tall: Record<Side, boolean> } {
...
function showsPost(wall: Block): boolean {
...
function refresh(wall: Block): void {
  const states = wall.permutation.getAllStates();
  ...
  if (changed) {
    const updated: States = { ...states, [POST_COVER]: post };
    for (const side of SIDES) {
...
function refreshWallAt(dimension: Dimension, position: Vector3): void {
  try {
...
function refreshAround(dimension: Dimension, position: Vector3): void {
...
function scheduleRefresh(dimension: Dimension, position: Vector3): void {
  const { x, y, z } = position;
