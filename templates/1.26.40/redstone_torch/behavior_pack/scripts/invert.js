...
function supportIsPowered(block, face) {
...
function tripsBurnout(key, tick) {
    const changes = (recentChanges.get(key) ?? []).filter((t) => tick - t < BURNOUT_WINDOW_TICKS);
    changes.push(tick);
...
system.beforeEvents.startup.subscribe((init) => {
    init.blockComponentRegistry.registerCustomComponent("kai_templates:redstone_torch_invert", {
        ...
        onTick(event) {
            ...
            if (wanted === lit) {
                ...
                if (lit && Math.random() < DUST_CHANCE) {
                    ...
                    block.dimension.spawnParticle("minecraft:redstone_torch_dust_particle", {
                        x: block.x + flame.x,
                        y: block.y + flame.y,
                        z: block.z + flame.z,
                    });
