...
export function pickleCount(block: Block): number {
...
export function isDead(block: Block): boolean {
  return block.permutation.getAllStates()[DEAD_STATE] === true;
}

// Rewrite the block with a new count and water flag
export function setPickle(block: Block, count: number, dead: boolean): void {
  const states = block.permutation.getAllStates();
  block.setPermutation(
    BlockPermutation.resolve(PICKLE_ID, {
      ...states,
      [COUNT_STATE]: count - 1,
      [DEAD_STATE]: dead,
    }),
