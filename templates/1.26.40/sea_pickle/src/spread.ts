...
function canSeatPickle(block: Block | undefined): boolean {
...
function spreadAround(origin: Block): void {
  for (let attempt = 0; attempt < SPREAD_ATTEMPTS; attempt += 1) {
    const dx =
      Math.floor(Math.random() * (SPREAD_RADIUS * 2 + 1)) - SPREAD_RADIUS;
    const dz =
      Math.floor(Math.random() * (SPREAD_RADIUS * 2 + 1)) - SPREAD_RADIUS;
    if (dx === 0 && dz === 0) {
      continue;
    }
    let seat: Block | undefined;
    try {
