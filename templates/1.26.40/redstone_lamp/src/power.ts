...
const goingDark = new Map<string, number>();
...
function isLit(block: Block): boolean {
...
function cancelDarkening(key: string): void {
...
system.beforeEvents.startup.subscribe((init) => {
  init.blockComponentRegistry.registerCustomComponent(
    "kai_templates:redstone_lamp_power",
    {
      ...
      onRedstoneUpdate(event: BlockComponentRedstoneUpdateEvent): void {
        ...
        const handle = system.runTimeout(() => {
          ...
          if (
            current?.typeId === LAMP_ID &&
            current.permutation.getAllStates()[LIT_STATE] === true
          ) {
            current.setPermutation(
              BlockPermutation.resolve(LAMP_ID, { [LIT_STATE]: false }),
            );
          }
        }, OFF_DELAY_TICKS);
