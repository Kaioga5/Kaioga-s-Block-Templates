...
function findHead(block: Block): Block | undefined {
    let current = block;
...
function heightOf(head: Block): number {
    let height = 1;
...
function grow(head: Block): void {
    ...
    above.setPermutation(
        BlockPermutation.resolve(DRIPLEAF_ID, { ...states, [HEAD_STATE]: true, [TILT_STATE]: "none" }),
    );
    head.setPermutation(
        BlockPermutation.resolve(DRIPLEAF_ID, { ...states, [HEAD_STATE]: false, [TILT_STATE]: "none" }),
    );
}
...
function cascade(dimension: Dimension, cell: Vector3, step: number): void {
    ...
    init.blockComponentRegistry.registerCustomComponent("kai_templates:dripleaf_growth", {
        ...
        onPlace(event: BlockComponentOnPlaceEvent): void {
            ...
            below.setPermutation(
                BlockPermutation.resolve(DRIPLEAF_ID, {
                    ...states,
                    [HEAD_STATE]: false,
                    [TILT_STATE]: "none",
                }),
