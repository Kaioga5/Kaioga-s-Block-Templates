...
const stripTargets = new Map<string, string>([
  ["kai_templates:log", "kai_templates:stripped_log"],
]);

// Strip the log and charge the axe
function strip(block: Block, strippedId: string, player: Player): void {
  ...
  block.setPermutation(
    BlockPermutation.resolve(strippedId, block.permutation.getAllStates()),
  );
