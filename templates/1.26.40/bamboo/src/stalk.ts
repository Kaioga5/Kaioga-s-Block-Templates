...
function section(thick: boolean, leaves: string, mature: boolean): BlockPermutation {
...
function setState(block: Block, state: string, value: boolean | string): void {
...
export function heightBelow(block: Block): number {
    let count = 0;
...
export function heightOf(top: Block): number {
    return heightBelow(top) + 1;
}

// Walk up to the block with nothing bamboo above it
export function findTop(block: Block): Block {
    let top = block;
...
export function nextSection(top: Block, mature: boolean): BlockPermutation {
...
function reshapeBelow(top: Block, thick: boolean): void {
    let part: Block | undefined = top.below();
...
export function settle(top: Block): void {
    const under = top.below();
...
export function growFrom(top: Block): boolean {
    const above = top.above();
