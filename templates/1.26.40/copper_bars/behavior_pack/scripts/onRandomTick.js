// Registers the component that weathers the copper bars over time.
// Copper advances through four looks, shiny, exposed, weathered, oxidized,
// on random ticks. Each stage is a separate block, so advancing a stage
// means swapping the block type; the engine recomputes the bar connections
// for the new block on its own. Waxed blocks never carry this component,
// so wax freezes the look with zero script cost.
import { system } from "@minecraft/server";
// Import the family table that maps each stage to the next one
import { STAGE_BY_ID } from "./family.js";
...
system.beforeEvents.startup.subscribe((init) => {
    init.blockComponentRegistry.registerCustomComponent("kai_templates:copper_bars_oxidize", {
        // Run this code on each random tick
        onRandomTick(event) {
            // Get the block from the event
            const { block } = event;
            // Find this block's stage and the stage after it. The final
            // stage has no next entry, so fully oxidized bars stop here
            ...
            // Swap to the next stage's block. Connection states are managed
            // by the engine and settle again right after the swap
            block.setType(stage.next);
        },
    });
});
