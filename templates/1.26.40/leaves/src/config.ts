...
export const supportIds = new Set<string>();
...
// What breaking or rotting leaves gives back, when the player was not using
// shears and had no Silk Touch. Each chance list is indexed by Fortune level,
// with the last entry reused above that, the same shape vanilla uses
export interface LeafDrop {
    // The item identifier
    id: string;
    // Chance to drop, indexed by Fortune level
    chances: number[];
    // How many drop when the roll succeeds
    min: number;
    max: number;
}

export const leafDrops: LeafDrop[] = [
